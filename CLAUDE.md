...
- **Перед записью в §13** — сначала определить следующую версию релиза
  из `version.properties` (VERSION_MAJOR.VERSION_MINOR.VERSION_PATCH),
  если правка тянет на отдельный релиз — увеличить VERSION_PATCH.
  Версия в записи §13 должна совпадать с той, что пойдёт в релиз.
