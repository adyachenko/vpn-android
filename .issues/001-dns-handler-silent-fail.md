...
Рекомендованный порядок инвестигации: ~~B~~ → C → D. A и B закрыты.

**Статус вариантов (2026-04-21):**
- **A:** закрыт (traffic-storm из v1.2.17)
- **B:** закрыт — эмпирически подтверждено (v1.2.18, 2026-04-20), что
  `Network.bindSocket()` к VPN network из excluded-процесса даёт EPERM
- **C:** остаётся как путь для pure-DNS failure (немой DNS-handler).
  Для частичной QUIC-деградации (§1.3b) теперь есть
  `ConnectionFailureDetector` (v1.2.19), но он НЕ покрывает DNS-only
  сбой, где TCP-стримы работают а DNS молчит
- **D:** не нужен если C реализован

**Новая находка (2026-04-21):** при анализе лога v1.2.18 (35K строк)
обнаружена **частичная QUIC-деградация** — отдельный класс сбоя от
DNS-handler death. urltest passes, DNS (DoH) resolves, но 150 из ~300
реальных TCP через hysteria2 получают `connection refused` с задержкой
1-31с. Это не issue #001 (DNS жив), но маскирует его — сложно отличить
«DNS молчит» от «QUIC refuse'ит соединение до DNS». v1.2.19 добавляет
`ConnectionFailureDetector` для этого кейса. Issue #001 (pure DNS death)
по-прежнему не детектится — нужен variant C.
